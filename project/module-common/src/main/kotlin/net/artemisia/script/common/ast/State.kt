...
sealed class State {
    ...
    data class Program(val body: List<State>, val location: BigLocation) : State()
    ...
    data class VariableDeclaration(val id: Expr.Identifier, val init: Expr?, val type: Expr?,val const: Boolean, val location: BigLocation) : State()
    data class ImportState(val file: Expr, val location: BigLocation) : State()
    ...
    data class FunctionDeclaration(val identifier: Expr.Identifier, val params: List<VariableDeclaration>, val returnValue: Expr, val body: BlockState?, val location: BigLocation) : State()
