...
class Decompilation(val file: File) {
    ...
    private fun function() {
        ...
        for (i in func.keys) {
            ...
            for (c in codes.keys) {
                println(
                    "       $c - ${(codes[c] as ArrayList<*>)[0]} : ${(codes[c] as ArrayList<*>)[1]} (${
                        if ((codes[c] as ArrayList<*>)[0] == "CreateFunction") parser.functions[(codes[c] as ArrayList<*>)[1].toString()
                            .toInt()] else pool[(codes[c] as ArrayList<*>)[1].toString().toInt()]
                    })"
                )
            }
